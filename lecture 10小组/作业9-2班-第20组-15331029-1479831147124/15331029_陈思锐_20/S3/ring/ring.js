...
function control_ring_onclick(event, call_after_send_request, call_after_get_responsed) {
	...
	$.ajax({url:"/", cache: false, success:function(data) {
		...
		if (!call_after_send_request) {turn_on_rings(is_already_on);}
		if ($("#control-ring li").filter(function(index, current_li) {
				return $(current_li).hasClass("turnOn");
			}).length === 0) turn_on_info_bar();
		if (!!call_after_get_responsed) call_after_get_responsed();
	}});
	if (!!call_after_send_request) call_after_send_request();
}
